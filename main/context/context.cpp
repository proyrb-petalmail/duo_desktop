...
namespace desktop
{
    ...
    void context::load_pipe_json(const std::string &pipe_json_name)
    {
        this->pipe_json_name = pipe_json_name;
        ifstream infile_stream(this->pipe_json_name); /* open pipe json */
        if (false == infile_stream.is_open())
        {
            Debug_Error("failed to open pipe json");
            throw error(error::error_enum::Open);
        }
        infile_stream >> json::wrap(this->pipe_json); /* read pipe json */
        Debug_Log(json::dump(this->pipe_json, {json::serializer::with_indent(4, ' ')}));
        infile_stream.close();
        Debug_Notice("load pipe json file successfully");
    }
